...
def getStateOfRoad(img,np_array, count):
    ...
    if (np_array == [[98,287], [136,55], [170,55], [450,287]]):
        ...
        cv2.imwrite("result1_/result1_" + str(count) + ".jpg", numpy_vertical)
    if (np_array == [[195,450], [595,180], [670,180], [625,450]]):
        ...
        numpy_horizontal2 = np.concatenate((blur, t), axis=1)
        numpy_vertical = np.concatenate((numpy_horizontal1, numpy_horizontal2), axis=0)
        cv2.imwrite("result2_/result2_" + str(count) + ".jpg", numpy_vertical)
    if (np_array == [[285,80], [185,83], [100,85], [0,140], [0,287], [511,287], [511,128], [450,100]]):
        ...
        numpy_horizontal2 = np.concatenate((blur, t), axis=1)
        numpy_vertical = np.concatenate((numpy_horizontal1, numpy_horizontal2), axis=0)
        cv2.imwrite("result3_/result3_" + str(count) + ".jpg", numpy_vertical)
    if (np_array == [[610,114], [583,135], [420,165], [140,450], [800,450], [665,170], [675,110]]):
        ...
        numpy_horizontal2 = np.concatenate((blur, t), axis=1)
        numpy_vertical = np.concatenate((numpy_horizontal1, numpy_horizontal2), axis=0)
        cv2.imwrite("result4_/result4_" + str(count) + ".jpg", numpy_vertical)
    return (capacity*100)
...
def getDataOfCamera(link, np_array, count):
    data ={}
    # link = "http://giaothong.hochiminhcity.gov.vn/render/ImageHandler.ashx?id=587c8183b807da0011e33d3f&t=1539764591446"
    # np_array = [[610,114], [583,135], [420,165], [140,450], [800,450], [665,170], [675,110]]
